...
Using the Spring Initializer (https://start.spring.io), create a new gradle spring boot project with the following
configurations:
...
Gradle.
By following a step-by-step process, we successfully converted the project structure, integrated Gradle as the build
tool, and configured frontend management using the org.siouan.frontend plugin.
Additionally, we ensured smooth execution of the application, copied the generated JAR to a separate folder, and
implemented a task to clean webpack-generated files.
Through these tasks, we gained valuable experience in Gradle usage and build automation, enhancing our understanding of
modern development practices. This assignment serves as a practical demonstration of Gradle's capabilities and its role
in facilitating efficient project management and development workflows.

## Alternative Solutions with Ant

# Ant Configuration

The main aim of this part of the class assignment is to practice Apache Ant by convert the basic version of the Tutorial
application.

## Ant Installation

1. Go to marketplace and install Ant.
...
3. Create a new branch named `tut-basic-ant` in your repository and switch to this branch.

    ```
    git checkout -b tut-basic-ant
    ```
4. Copy the contents of the basic tutorial into the working folder.

## Ant Configuration

5. create a build.xml file in the root of the project with the following content:
...
6. Build the project executing the following command:
    ```
    ant build
    ```
...
8. Copy the generated JAR to a folder named `dist` located at the project root level. Ensure this task is executed automatically by Ant after the `build` task. 
To do this add the following task to the `build.xml` file:
 ```
    <target name="copyJar">
...
9. Add a task to delete all files generated by webpack. Ensure this task is executed automatically by Ant before the `clean` task.
...
## Conclusion
In conclusion, Gradle and Apache Ant are both build automation tools, but they differ significantly in their approach and capabilities. 
Ant relies on XML for configuration, necessitating explicit task configuration and lacking built-in dependency management, while Gradle uses Groovy or Kotlin, offering a more concise syntax and powerful dependency management. 
Gradle also follows a convention-over-configuration paradigm, reducing the need for explicit configuration. 
Additionally, Gradle boasts a rich plugin ecosystem, superior performance optimizations, and strong community support compared to Ant. 
Ultimately, Gradle's advanced features and ease of use make it a preferred choice for many developers and projects.